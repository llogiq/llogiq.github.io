...
it soon. On the other hand, a very good static nullness check for Java has been introduced with the
FindBugs (now [SpotBugs](https://spotbugs.github.io/)) static analyzer
* Go, like lua has `nil`, but only for references. Since Go has value objects, those cannot be
`nil`

Verdict: `null` (or its cousin `nil`) has the effect of making *every* value an `Option`. By making
this type explicit, Rust (and Swift, and other modern languages) reduces the load it entails.
...
a count of the substrings before building or some kind of reallocation. Worse, until Java
