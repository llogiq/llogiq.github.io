...
It took me a bit to learn the macro syntax, but the [book](TODO) is really
helpful. I also perused a Stack Overflow
...
because as of 1.9.0, `catch_panic(_)` is stabilized, which allows us to catch
...
be made for all array sizes from zero to 32 elements. However, since the
benchmarks should be made for all sizes between 0 and 32, there's a lot of 
repetition. So I'm going to use another macro (*Warning: The following only 
works with a nightly Rust*):
...
Ok, let's pack up and call it a day.
