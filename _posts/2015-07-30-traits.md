...
2. <s>Your type is so big that copying it would be prohibitively expensive (e.g. 
   it could contain an `[f64; 65536]`)</s> Rust Demigod eddyb notes that Rust
   would still copy the whole thing unless you work with references. However,
   he also gives a better reason not to Copy: If your type is an 
   [`Iterator`](http://doc.rust-lang.org/std/iter/trait.Iterator.html) (to 
   which we will return later), marking it as `Copy` would induce needless
   copying.
