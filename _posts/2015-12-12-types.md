...
Now if we write `<<Not<True>::Output as Same<False>>::Output` in our code, we will get a `False` type and a compile time assertion that ¬⊤ is indeed the same as ⊥. On the other hand, `<True as Same<Unknown>>::Output` will result in a compiler error. So we can coax the type system into proving assertions for us.
...
Now `<Not<True>::Output::to_ternary()` will be a (constant) `Ternary::F` (be sure to inline as in the above code!).
...
I'll release the code for the ternary logic on github soon, so you can play with the code. I may also experiment a bit more with type data structures in the future, e.g. I have an idea on how to represent something b-tree like within types, which could pave the way for other funny things.
